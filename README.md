...
- 当前 Flutter 版仓库已经补齐 `v*` tag 触发的独立 GitHub Actions 发布流，可产出 `macOS amd64 / arm64 / universal` 的 `zip + dmg`，以及 `Windows amd64 / arm64` 的 `zip + installer.exe`。
- 发布入口位于 [`.github/workflows/release-macos-amd64.yml`](./.github/workflows/release-macos-amd64.yml)、[`.github/workflows/release-macos-arm64.yml`](./.github/workflows/release-macos-arm64.yml)、[`.github/workflows/release-macos-universal.yml`](./.github/workflows/release-macos-universal.yml)、[`.github/workflows/release-windows-amd64.yml`](./.github/workflows/release-windows-amd64.yml) 和 [`.github/workflows/release-windows-arm64.yml`](./.github/workflows/release-windows-arm64.yml)，推送版本标签后会分别构建并发布对应 release 资源。
...
- 推送 `v*` tag 后，GitHub Actions 会并行触发 5 条独立发布流，分别处理 `macOS amd64`、`macOS arm64`、`macOS universal`、`Windows amd64` 和 `Windows arm64`。
- macOS 每条流都会发布对应架构的 `zip + dmg`；Windows 每条流都会发布对应架构的 `zip + installer.exe`。

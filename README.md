...
- 云端「我喜欢」歌单详情页现在会在后台定期强制刷新，新的云收藏会自动补进列表，并同步修正曲目红心状态。
...
- `make android-bridge` 会通过 Android NDK 构建 `arm64-v8a` 版 Go `c-shared` bridge，并同步到 Gradle `jniLibs` 输入目录。
...
- `make android-run` 会加载 `~/.zshrc` 和仓库根目录 `.env.local`，先构建并同步 Android bridge `.so`，再以 debug 模式启动 `emulator-5554`。

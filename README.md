...
- Android workflow release 现已支持稳定 keystore 签名；只要 GitHub Actions 配置同一套签名密钥，后续 release APK 就可以直接覆盖升级，不需要先卸载旧版。
- Android 移动端页面切换现在补上了横向淡入滑动过渡，返回键事件也会由宿主显式转发回 Flutter 的页面返回链路。
...
- 设置页现已接通 `歌曲海源` 渠道，支持真实搜索、试听地址解析和歌词抓取，并复用统一的 provider key / sourceId 命名空间。
- 设置页里的播放故障转移顺序已改为可视化排序，新接入音源会自动补到链尾，不再要求手填逗号分隔字符串。
...
- Android release 签名环境变量：
  ...
  `CP_ANDROID_KEY_ALIAS`、
  `CP_ANDROID_KEY_PASSWORD`。
  GitHub Actions 推荐直接配置为仓库 secrets，并让 workflow 在 release 构建时注入。
...
- `歌曲海源` 当前已接通真实搜索、播放地址解析和歌词抓取；每日推荐、收藏回写和更深的站点兼容性仍待后续补齐。

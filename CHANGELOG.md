...
- 推 tag 触发 release 时，发布 workflow 会直接按 tag 区间内的提交记录整理 release body：标题转小写后包含 `fix` 的归到 `问题修复`，其他归到 `更新内容`。
...
- 偏好设置新增“修复数据库”入口，会用独立子窗口确认并显示 loading；确认后会清理本地缓存的云歌单副本、清空酷狗云歌单缓存并切回离线模式，供后续重新切换时拉取干净的云歌单状态。
